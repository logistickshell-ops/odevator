  ...
  <section className="space-y-4 rounded-2xl sm:rounded-3xl border border-sky-100 bg-white p-4 sm:p-6 shadow-sm">
    <div className="flex flex-col justify-between gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:items-center sm:pb-4">
      <div className="min-w-0">
        <h3 className="flex items-center gap-2 text-sm sm:text-base font-black text-slate-800">
          ...
          {tr("Погода для рекомендации")}
        </h3>
        <p className="mt-0.5 text-[10px] sm:text-xs text-slate-400">
          {tr("Обычно мы используем погоду выбранного города. При необходимости её можно задать вручную.")}
        </p>
      </div>
    ...
    {isManual && (
      <div className="space-y-4 rounded-2xl border border-rose-100 bg-rose-50/55 p-3 sm:p-4 animate-fadeIn">
        <p className="text-[10px] sm:text-xs font-bold text-rose-700">{tr("Изменения влияют только на текущий просмотр и не сохраняют погоду города.")}</p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
          <label className="space-y-2 rounded-xl border border-slate-100 bg-white p-3">
            <span className="flex justify-between text-[10px] sm:text-xs font-bold text-slate-600"><span className="flex items-center gap-1"><Thermometer size={12} className="text-rose-500" />{tr("Температура")}</span><span className="text-sky-700">{temp > 0 ? `+${temp}` : temp}°C</span></span>
          ...
          <label className="space-y-2 rounded-xl border border-slate-100 bg-white p-3">
            <span className="flex justify-between text-[10px] sm:text-xs font-bold text-slate-600"><span className="flex items-center gap-1"><Wind size={12} className="text-slate-500" />{tr("Ветер")}</span><span className="text-sky-700">{windSpeed} {tr("км/ч")}</span></span>
            ...
            <span className="flex justify-between text-[9px] font-bold text-slate-400"><span>0</span><span>{tr("Умеренный")}</span><span>50</span></span>
          </label>
          <label className="space-y-2 rounded-xl border border-slate-100 bg-white p-3">
            <span className="flex justify-between text-[10px] sm:text-xs font-bold text-slate-600"><span className="flex items-center gap-1"><Droplets size={12} className="text-sky-500" />{tr("Влажность")}</span><span className="text-sky-700">{humidity}%</span></span>
        ...
        <div className="rounded-xl border border-slate-100 bg-white p-3">
          <span className="mb-2 block text-[10px] sm:text-xs font-bold text-slate-600">{tr("Погода")}</span>
          <div className="grid grid-cols-4 gap-2">
            {(['sunny', 'cloudy', 'rainy', 'snowy'] as const).map((condition) => (
              ...
              >
                ...
                <span>{condition === 'sunny' ? tr("Ясно") : condition === 'cloudy' ? tr("Облачно") : condition === 'rainy' ? tr("Дождь") : tr("Снег")}</span>
              </button>
            ))}
          </div>
        </div>
