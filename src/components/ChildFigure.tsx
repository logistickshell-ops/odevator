  ...
  return (
    <svg viewBox="0 0 240 400" className="h-full w-full select-none" role="img" aria-label="Иллюстрация ребёнка по погоде">
      <defs>
        ...
        {/* ИСПРАВЛЕНИЕ: добавлен градиент hi для бликов на одежде */}
        <linearGradient id={gid('hi')} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.35" />
          <stop offset="55%" stopColor="#FFFFFF" stopOpacity="0" />
        </linearGradient>
        
        {/* ИСПРАВЛЕНИЕ: добавлен градиент ground для тени под ногами */}
      ...
      {/* Фон и атмосферные эффекты */}
      <ellipse cx={CX} cy={190} rx="112" ry="150" fill={`url(#${gid('aura')})`} />
      {/* ИСПРАВЛЕНИЕ: тень под ногами */}
          ...
          <g filter={`url(#${uid}-soft)`}>
            {drawSkirt ? (
              <>
                ...
                {/* ИСПРАВЛЕНИЕ: теперь использует правильный gid('hi') */}
                <path d={`M ${CX - 22} ${Y_WAIST - 8} L ${CX + 22} ${Y_WAIST - 8} L ${CX + 44} ${Y_WAIST + 58} Q ${CX} ${Y_WAIST + 72} ${CX - 44} ${Y_WAIST + 58} Z`} fill={`url(#${gid('hi')})`} opacity="0.3" />
              </>
            ) : drawShorts ? (
              <>
...
{/* Слой 6: Обувь */}
          ...
          <g filter={`url(#${uid}-soft)`}>
            {isRainy && !cold ? (
              <>
                ...
                <rect x={CX - 35} y={Y_ANKLE + 20} width="31" height="8" rx="4" fill="#92400E" opacity="0.75" />
                <rect x={CX + 4} y={Y_ANKLE + 20} width="31" height="8" rx="4" fill="#92400E" opacity="0.75" />
              </>
            ) : cold ? (
              <>
                {/* Зимние ботинки */}
        ...
        {/* Слой 2: Верхний слой (худи/свитер) */}
        ...
        {show.outer && (
          <g filter={`url(#${uid}-soft)`}>
            ...
            {(zone === 'arctic' || zone === 'winter') && !isRainy && (
              <g stroke={`url(#${gid('hat')})`} strokeWidth="2" opacity="0.4" fill="none" strokeLinecap="round">
            ...
            <g filter={`url(#${uid}-soft)`}>
              {hot ? (
                ...
                <>
                  ...
                  <rect x={CX - 36} y={Y_HEAD - 18} width="72" height="14" rx="7" fill={`url(#${gid('hat')})`} />
