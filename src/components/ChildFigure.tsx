...
interface ChildFigureProps {
  ...
  isSnowy: boolean;
  isWindy: boolean;
  show: LayerVisibility; // Объект управления видимостью всех 7 слоев
}

// Зоны температуры для выбора типа одежды
...
export const ChildFigure: React.FC<ChildFigureProps> = ({ 
  ...
  // Пути для конечностей (Bezier curves)
  ...
  return (
    <svg viewBox="0 0 240 400" className="h-full w-full select-none">
      ...
      <g className="animate-breathe">
        {/* ВОЛОСЫ (задняя часть, только для девочек) */}
        ...
        {/* СЛОЙ 6: ОБУВЬ (зависит от погоды и сезона) */}
        {show.shoes && (
          <g filter={`url(#${uid}-soft)`}>
            {isRainy && !isCold ? (
              // ДОЖДЬ: Желтые резиновые сапоги
              <>
              ...
              // ХОЛОД: Темные зимние ботинки со снегом
              <>
              ...
              // ЖАРА: Легкие сандалии (цвет из палитры)
              <>
        ...
        {/* СЛОЙ 4: ВЕРХНЯЯ ОДЕЖДА (куртка, пуховик, комбинезон) */}
        {show.outer && (
          <g filter={`url(#${uid}-soft)`}>
        ...
        {/* СЛОЙ КОЖИ: Торс и руки (базовый слой под одеждой) */}
        ...
        <g>
          ...
          {/* Волосы спереди */}
          <g fill={hairColor}>
            {girl ? <path d={`M ${CX-35} ${Y_HEAD-5} Q ${CX} ${Y_HEAD-25} ${CX+35} ${Y_HEAD-5} Q ${CX+38} ${Y_HEAD-35} ${CX} ${Y_HEAD-38} Q ${CX-38} ${Y_HEAD-35} ${CX-35} ${Y_HEAD-5} Z`} />
          ...
          <circle cx={CX+20} cy={Y_HEAD+13} r={isCold ? 8 : 6} fill="#FF9E9E" opacity="0.6" />
          {/* Рот (улыбка или открытый рот на жаре) */}
        ...
        {/* СЛОЙ 5: ГОЛОВНОЙ УБОР (шапка, панама, кепка) */}
        {show.headwear && (
          <g>
        ...
        {/* СЛОЙ 7: АКСЕССУАРЫ (варежки, шарф) */}
