...
export const AvatarVisualizer: React.FC<AvatarVisualizerProps> = ({
  ...
  const [layerVisibility, setLayerVisibility] = useState<LayerVisibility>({
    underwear: true, lower: true, upper: true, outer: true, shoes: true, headwear: true, accessory: true,
  });
  ...
  const renderLayerCard = (title: string, icon: React.ReactNode, items: ClothingItem[], isActive: boolean, layerNum: number) => {
    if (!items || items.length === 0) return null;
    const layerColor = layerNum === 3 ? 'bg-rose-500' : layerNum === 2 ? 'bg-violet-500' : layerNum === 1 ? 'bg-sky-500' : 'bg-amber-500';

    return (
      <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 transition-all duration-300 ${isActive ? 'bg-white border-indigo-200/80 shadow-sm' : 'bg-slate-50/50 border-slate-100 opacity-60'}`}>
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 mb-2 sm:mb-3">
          <span className={`flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg sm:rounded-xl text-white font-extrabold text-[10px] sm:text-xs shrink-0 ${layerColor}`}>{icon}</span>
        ...
        <div className="space-y-1.5 sm:space-y-2.5">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} onClick={() => handleItemClick(item)} className={`group cursor-pointer p-2.5 sm:p-3.5 rounded-lg sm:rounded-xl border-2 transition-all duration-200 active:scale-[0.98] ${isHl ? 'border-indigo-400 bg-indigo-50/40 shadow-sm' : 'border-slate-100 bg-white hover:border-slate-200'}`}>
                <div className="flex items-start gap-2 sm:gap-3">
                  <span className="text-2xl sm:text-3xl select-none leading-none shrink-0 mt-0.5 p-1 sm:p-1.5 bg-slate-50 rounded-lg sm:rounded-xl border border-slate-100">{item.emoji}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    {item.tips && <div className="mt-1.5 sm:mt-2 flex items-start gap-1 sm:gap-1.5 text-[9px] sm:text-[10px] text-amber-700 bg-amber-50 p-1.5 sm:p-2 rounded-lg border border-amber-100/70"><Info size={10} className="mt-0.5 shrink-0 text-amber-500" /><span className="leading-relaxed">{item.tips}</span></div>}
                  </div>
                </div>
              </div>
            );
  ...
  const getItemsByLayer = (layer: string): ClothingItem[] => {
    if (outfit.layers) return (outfit.layers as any)[layer] || [];
    return [];
  };
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 sm:gap-8 items-start">
        ...
        <div className={`bg-gradient-to-b ${bgGradient} rounded-2xl sm:rounded-3xl p-3 sm:p-5 border-3 sm:border-4 border-white shadow-lg sm:shadow-xl relative flex flex-col items-center overflow-hidden`}>
          <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-10">
            <div className="bg-white/90 backdrop-blur px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-extrabold text-slate-700 border border-white/80 shadow-sm flex items-center gap-1.5 sm:gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-lg text-white text-[10px] sm:text-[11px] ${climateLabel.color}`}>{effectiveTemp > 0 ? '+' : ''}{effectiveTemp}°</span>
            </div>
          </div>
          <div className="absolute top-2 sm:top-3 right-2 sm:right-3 z-10">
            <button onClick={() => setLayerVisibility({ underwear: true, lower: true, upper: true, outer: true, shoes: true, headwear: true, accessory: true })} className="bg-white/90 backdrop-blur p-2 rounded-xl text-slate-500 active:text-indigo-600 active:bg-white transition border border-white/80 shadow-sm"><Layers size={14} /></button>
          </div>
          <div className="w-full max-w-[230px] sm:max-w-[280px] aspect-[11/20] pt-6 sm:pt-8 pb-1 sm:pb-2">
            <ChildFigure gender={gender} effectiveTemp={effectiveTemp} isRainy={isRainy} isSnowy={isSnowy} isWindy={isWindy} show={layerVisibility} />
          </div>
          <div className="mb-1 sm:mb-2 flex items-center gap-1.5 sm:gap-2">
            <span className={`text-xs sm:text-sm font-extrabold ${gender === 'girl' ? 'text-pink-600' : 'text-blue-600'}`}>{gender === 'girl' ? '👧 Девочка' : '👦 Мальчик'}</span>
            <span className="text-[10px] sm:text-xs text-slate-400">•</span>
            <span className="text-[10px] sm:text-xs font-bold text-slate-500">{effectiveTemp <= 0 ? '️ Зима' : effectiveTemp <= 10 ? '🍂 Демисезон' : '☀️ Лето'}</span>
          </div>
        </div>
      ...
      <div className="w-full lg:col-span-7 space-y-3 sm:space-y-5">
        <div className="hidden sm:block">
          <h3 className="text-xl font-extrabold text-slate-800 flex items-center gap-2"><Layers size={22} className="text-indigo-500" /><span>Полный гардероб на прогулку</span></h3>
          <p className="text-sm text-slate-500 mt-1">Собираем ребенка слой за слоем.</p>
        </div>
        <div className="sm:hidden text-center"><h3 className="text-sm font-extrabold text-slate-800">📋 Чек-лист гардероба</h3></div>
        ...
        {outfit.specialAdvice && outfit.specialAdvice.length > 0 && (
          <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-amber-50/80 border border-amber-200/70 space-y-1.5 sm:space-y-2">
            <h4 className="font-extrabold text-amber-900 text-[11px] sm:text-sm flex items-center gap-1.5 sm:gap-2"><span className="text-sm sm:text-base">⚠️</span> Важные рекомендации</h4>
            <ul className="space-y-1">{outfit.specialAdvice.map((advice: string, i: number) => (<li key={i} className="text-[10px] sm:text-xs text-amber-800 leading-relaxed flex items-start gap-1.5 sm:gap-2"><span className="mt-0.5 shrink-0 text-amber-500">▸</span><span>{advice}</span></li>))}</ul>
          </div>
        )}
      </div>
    </div>
