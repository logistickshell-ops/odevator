...
const QUICK_NOTES = [
  tr("Положить сменные носки"),
  tr("Взять воду"),
  tr("Добавить светоотражатель"),
  tr("Положить дополнительный слой"),
];
    ...
    <section className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-sm">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-base sm:text-xl font-black text-slate-800">{tr("Мои заметки к прогулке")}</h3>
          <p className="text-[11px] sm:text-xs text-slate-500 mt-0.5 leading-relaxed">{tr("Коротко зафиксируйте то, что важно не забыть. Заметки сохраняются только на этом устройстве.")}</p>
        </div>
      </div>

      <div className="mt-4 rounded-xl border border-violet-100 bg-violet-50/50 px-3 py-2 text-[10px] sm:text-xs text-violet-800 font-semibold">{tr("Сейчас:")} {currentContext}</div>
      ...
      <div className="mt-3 flex flex-col sm:flex-row gap-2">
        <label className="sr-only" htmlFor="walk-note">{tr("Заметка к прогулке")}</label>
        <textarea
          id="walk-note"
          value={draft}
          maxLength={MAX_LENGTH}
          onChange={(event) => setDraft(event.target.value)}
          placeholder={tr("Например: взять запасные варежки и термос")}
        ...
        <button onClick={addNote} disabled={!draft.trim()} className="sm:self-end inline-flex items-center justify-center gap-1.5 rounded-xl bg-violet-600 px-3.5 py-2.5 text-xs font-extrabold text-white shadow-sm transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:bg-slate-200">
          {saved ? <Check size={16} /> : <BookmarkPlus size={16} />}{saved ? tr("Сохранено") : tr("Сохранить")}
        </button>
      </div>
      ...
      {notes.length > 0 ? (
        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between gap-2"><h4 className="text-[11px] sm:text-xs font-black text-slate-700">{tr("Ближайшие напоминания")}</h4><span className="text-[10px] text-slate-400">{tr("до")} {MAX_NOTES} {tr("записей")}</span></div>
          {notes.map((note) => (
            <div key={note.id} className="group flex items-start gap-2.5 rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2.5">
              ...
              <button onClick={() => removeNote(note.id)} className="rounded-lg p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition" title={tr("Удалить заметку")} aria-label={tr("Удалить заметку")}><Trash2 size={14} /></button>
            </div>
          ))}
        </div>
      ) : (
        <p className="mt-4 text-[11px] sm:text-xs text-slate-400">{tr("Пока нет личных заметок. Используйте быстрый шаблон или напишите свою.")}</p>
      )}
    </section>
  );
};
