...
interface ChildFigureProps {
  ...
  isSnowy: boolean;
  isWindy: boolean;
  show: LayerVisibility; // Новый пропс для управления слоями
}
...
export const ChildFigure: React.FC<ChildFigureProps> = ({
  ...
  // Логические флаги для автоматического выбора аксессуаров внутри SVG
  ...
  // Кривые Безье для анатомичных рук и ног
          ...
          <g filter={`url(#${uid}-soft)`}>
            {drawSkirt ? (
              <>
                ...
                <path d={`M ${CX - 22} ${Y_WAIST - 8} L ${CX + 22} ${Y_WAIST - 8} L ${CX + 44} ${Y_WAIST + 58} Q ${CX} ${Y_WAIST + 72} ${CX - 44} ${Y_WAIST + 58} Z`} fill="url(#g-hi)" opacity="0.3" />
              </>
            ) : drawShorts ? (
              <>
